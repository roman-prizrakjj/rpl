...
import { LucideQrCode, LucideSettings, LucideMapPin, LucideTrophy, LucideCalendar, LucideLogOut, LucideChevronRight, LucideCheckCircle } from 'lucide-react';
  ...
  return (
    <div className="p-4 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative mb-8 group">
        ...
        <div className="relative bg-[#0b0f19] border border-gray-800 rounded-[2rem] overflow-hidden">
          {/* Card Header */}
          ...
          <div className="p-6 flex gap-6">
            <div className="relative">
              <img 
                ...
                alt="Avatar"
              />
              <div className="absolute -bottom-2 -left-2 bg-green-500 text-black text-[8px] font-black px-2 py-0.5 rounded-md border-2 border-[#0b0f19]">
                VERIFIED
              </div>
            </div>
            <div className="flex-1 flex flex-col justify-center">
              <h3 className="text-lg font-black leading-tight mb-1 text-white">{user.name}</h3>
          ...
          <div className="px-6 py-4 border-t border-gray-800/50 bg-gray-900/30 flex justify-between items-center">
             <div className="flex -space-x-2">
                {[1,2,3].map(i => (
                  <div key={i} className="w-6 h-6 rounded-full border-2 border-[#0b0f19] bg-gray-800 flex items-center justify-center overflow-hidden">
                    <img src={`https://picsum.photos/seed/club${i}/40/40`} className="w-full h-full object-cover" />
                  </div>
                ))}
                <div className="w-6 h-6 rounded-full border-2 border-[#0b0f19] bg-blue-600 text-[8px] flex items-center justify-center font-bold text-white">
                  +12
                </div>
             </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        <div className="bg-gray-800/40 border border-gray-700 p-4 rounded-3xl transition-all hover:border-blue-500/30">
        ...
        <div className="bg-gray-800/40 border border-gray-700 p-4 rounded-3xl transition-all hover:border-blue-500/30">
          <LucideCheckCircle className="w-5 h-5 text-green-400 mb-2" />
          <div className="text-xl font-black text-white">{user.totalTasksDone}</div>
          <div className="text-[10px] text-gray-500 font-bold uppercase">Заданий выполнено</div>
        </div>
        <div className="bg-gray-800/40 border border-gray-700 p-4 rounded-3xl transition-all hover:border-blue-500/30">
      ...
      <button className="w-full py-4 flex items-center justify-center gap-2 text-red-500 font-bold uppercase tracking-widest text-xs hover:bg-red-500/5 rounded-2xl transition-colors">
